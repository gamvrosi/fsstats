counts <- c("data/busyness_msr1", "data/busyness_msr2",
            "data/busyness_msr3");
...
mtext(text="Fraction of busy periods", side=1, line=2.5)
mtext(text="Fraction of total busy periods", side=2, line=2.5)
...
for (i in seq(1, length(ecdfs))) {
  ...
  # 		  col=lcolors[i]);
  llabels <- c(llabels, paste("Total busy period (us) = ", sum(as.numeric(data[[i]])), sep=""));
}
